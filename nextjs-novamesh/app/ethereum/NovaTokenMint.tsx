...
import { useReadNovaExchangeNovaToken } from "@/src/generated";

export default function NovaTokenMint() {
  const { address } = useAccount();
  const novaToken = useReadNovaExchangeNovaToken();
  return (
    <>
      <div
      ...
      >
        ...
        {address ? <h3 className="mb-3">Current Balance: {novaToken.data} </h3> : <div></div>}
        {address ? <button
